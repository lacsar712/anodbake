...
func (c *Controller) Tick(ctx context.Context, snap model.PlantSnapshot, firing bool) (model.PitlineReading, error) {
	select {
	case <-ctx.Done():
		return snap.Pitline, fmt.Errorf("%w", model.ErrContextDone)
	default:
	}
	pressure, err := c.pressure.Compute(snap, firing)
	if err != nil {
		return snap.Pitline, err
	}

...
func (s *PlantStore) UpdateAnodepit(unitID string, anodepit model.AnodepitReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
...
func (s *PlantStore) UpdatePitfire(unitID string, comb model.PitfireReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
...
func (s *PlantStore) UpdatePitline(unitID string, pitline model.PitlineReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {

...
func (a *AirflowBalancer) IgnitionRate(settings model.PlantSettings) float64 {
	return settings.PitchFlowTPH * 18
}

func (a *AirflowBalancer) Compute(snap model.PlantSnapshot) float64 {
	stoich := snap.Pitfire.PitchFlowTPH * 15.5
	o2Factor := 1 + (snap.Settings.ExcessO2Setpoint / 100)
	return stoich * o2Factor
}

func (a *AirflowBalancer) ExcessO2(reading model.PitfireReading) float64 {
	if reading.AirflowTPH <= 0 {
		return 0
	}
	stoichAir := reading.PitchFlowTPH * 15.5
	if stoichAir <= 0 {
		return 21.0
	}
...
func (a *AirflowBalancer) WithinLimits(reading model.PitfireReading) bool {
	o2 := reading.ExcessO2Pct
	return o2 >= model.MinPitframeO2Percent && o2 <= model.MaxPitframeO2Percent
}
